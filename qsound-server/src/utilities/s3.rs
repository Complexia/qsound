...
pub async fn fetch_from_s3(request: String) -> Result<Vec<u8>> {
    ...
    let bytes_as_value: Value =
        match crate::entities::pyspaces::download_from_s3::<Value>(request).await {
            Ok(val) => val,
...
pub async fn get_presigned_link_for_upload(filename: String) -> Result<String> {
    let presigned_link = crate::entities::pyspaces::get_presigned_link_for_upload(filename).await?;
    Ok(presigned_link)
}

pub async fn get_presigned_link_for_download(filename: String) -> Result<String> {
    let presigned_link =
        crate::entities::pyspaces::get_presigned_link_for_download(filename).await?;
    Ok(presigned_link)
}
