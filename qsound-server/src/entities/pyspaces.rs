...
pub async fn upload_to_s3<T: serde::Serialize>(request: &T) -> Result<serde_json::Value> {
    ...
    let song_as_bytes = song.as_bytes().to_owned();
    ...
    let pyspaces_endpoint = crate::utilities::get_env_variable("PYSPACES_ENDPOINT", "");
    let endpoint = format!("{}/get-presigned-link-for-upload", pyspaces_endpoint);
...
pub async fn download_from_s3<T: serde::Serialize>(request: String) -> Result<serde_json::Value> {
    let entity = "pyspaces";
...
pub async fn get_presigned_link_for_upload(filename: String) -> Result<String> {
    let entity = "pyspaces";
...
pub async fn get_presigned_link_for_download(filename: String) -> Result<String> {
    let entity = "pyspaces";
    ...
    let pyspaces_endpoint = crate::utilities::get_env_variable("PYSPACES_ENDPOINT", "");
    let endpoint = format!("{}/get-presigned-link-for-download", pyspaces_endpoint);
